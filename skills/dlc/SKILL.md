...
| 4d | Address self-review | auto | Read the sub-agent's review comments. Fix every blocking/required finding (the highest-severity tiers in whatever scheme the review skill used) in follow-up commit(s); for lower-severity findings, apply judgment — address or reply with a brief rationale. Push the fixes, then for **every** comment reply with what was done **and resolve the thread** — resolving is mandatory, not "resolve or reply". An addressed-but-unresolved comment leaves the human unable to tell what still needs attention, which defeats the point of the self-review. Resolve a thread once it has been dealt with (code fixed, or replied to with a rationale for not acting); leave it unresolved only when it genuinely still needs the human's input. GitHub review threads can only be resolved via GraphQL — `gh` has no native command, which is why this step gets skipped; see **Resolving review threads** below. Do **not** spawn another review pass or mark the PR ready. → 4e. |
...
**Self-review with fresh eyes.** The 4c reviewer is a *new* sub-agent with no memory of this session — reviewing your own work in-context misses what you already believe is correct. It annotates the PR; you address at 4d (Critical/required fixed before 4e, lesser findings get a fix or a brief reply) and then **resolve every thread you dealt with** — a reply alone is not enough, because the human scanning the PR needs "unresolved" to mean "still open". The PR stays draft throughout.
