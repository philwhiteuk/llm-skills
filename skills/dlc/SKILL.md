...
When a step says "delegate to the X skill", that is an instruction to **actually invoke that skill via the Skill tool** (e.g. `develop:spec`, `develop:plan`, `develop:slack-message`) and then follow whatever instructions it returns — verbatim, at runtime.
...
| 1b | Track issue | auto | **Invoke the `develop:spec` skill via the Skill tool** and follow its current instructions to produce the issue description. _(Why: it owns the spec format and work-type classification — do not write the description yourself or from this file's description of it.)_ |
...
| 5b | Notify team | auto | **Invoke the `develop:slack-message` skill via the Skill tool** and follow its current instructions to compose and send a Slack message linking the PR and issue. Do not format the message yourself. |
