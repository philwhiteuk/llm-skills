...
| [no-word-salad](skills/no-word-salad/SKILL.md) | Write the minimum that does the job, in plain words: say it out loud, lead with the ask, one idea per sentence, cut what nobody asked, never narrate the visible. Referenced by `dlc` at self-review as a second lens over the diff; also invokable directly to scrub a draft. |
| [walkthrough](skills/walkthrough/SKILL.md) | Teach a human an unfamiliar codebase by walking the code path a ticket, bug report, PR under review, or prose-described change implicates — one hop per turn, reader-paced, with every structural claim carrying a clickable `path:line`. Calibrates to the reader first, walks outermost-inward, and treats a hop as a transfer of responsibility rather than a file. Pure understanding: it never proposes a change. User-invoked only. |
<!-- SKILLS-INDEX:END -->
